...
struct io_block_context {
    ...
    /*! NoC-S3 driver map/unmap APIs. */
    struct mod_noc_s3_memmap_api *noc_s3_memmap_api;
    /*! ATU map/unmap APIs. */
    ...
    uint8_t *region_idx)
{
    ...
    return ctx.noc_s3_memmap_api->map_region_in_psam(
    ...
    uint8_t region_idx)
{
    ...
    return ctx.noc_s3_memmap_api->unmap_region_in_psam(
        &dev, &comp_config, region_idx);
}
...
static int mod_io_block_bind(fwk_id_t id, unsigned int round)
{
    ...
    return fwk_module_bind(
        ctx.config->noc_s3_id, ctx.config->noc_s3_api_id, &ctx.noc_s3_memmap_api);
}
