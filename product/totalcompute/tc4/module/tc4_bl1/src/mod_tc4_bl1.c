...
struct tc4_bl1_ctx {
    const struct mod_tc4_bl1_config *bl1_config;
...
static int tc4_signal_error(fwk_id_t service_id)
{
    fwk_unexpected();
    return FWK_E_PANIC;
}
...
static int tc4_signal_message(fwk_id_t service_id)
{
    struct fwk_event event = {
        .source_id = FWK_ID_MODULE(FWK_MODULE_IDX_TC4_BL1),
        .target_id = FWK_ID_MODULE(FWK_MODULE_IDX_TC4_BL1),
        .id = FWK_ID_EVENT(FWK_MODULE_IDX_TC4_BL1, MOD_BL1_EVENT_RSS_HANDSHAKE),
    };
...
static const struct mod_transport_firmware_signal_api
    tc4_system_transport_firmware_signal_api = {
        .signal_error = tc4_signal_error,
        .signal_message = tc4_signal_message,
    };
...
static int bl1_deferred_setup(void)
{
    ...
    FWK_LOG_INFO("[TC4_BL1] Pinging RSS");
    ctx.firmware_api->trigger_interrupt(FWK_ID_ELEMENT(
...
static int tc4_bl1_bind(fwk_id_t id, unsigned int round)
{
    int status;

    /* Use second round only (round numbering is zero-indexed) */
    if (round == 1) {
        /* Bind to the Power Domain module */
...
static int tc4_bl1_start(fwk_id_t id)
{
    struct fwk_event event = {
        .source_id = FWK_ID_MODULE(FWK_MODULE_IDX_TC4_BL1),
        .target_id = FWK_ID_MODULE(FWK_MODULE_IDX_TC4_BL1),
        .id = FWK_ID_EVENT(FWK_MODULE_IDX_TC4_BL1, MOD_BL1_EVENT_RUN),
    };

    return fwk_put_event(&event);
}

static int tc4_bl1_process_event(
    ...
    struct fwk_event *resp)
{
    ...
    if (fwk_id_get_event_idx(event->id) == MOD_BL1_EVENT_RUN) {
        ...
        struct fwk_event systop_on_event = {
            ...
            .id = mod_tc4_bl1_notification_id_systop,
        ...
        FWK_LOG_INFO("[TC4_BL1] Got ACK from RSS");

        /* Power on the primary cluster and cpu using power domain */
        ctx.mod_pd_restricted_api->set_state(
            ctx.bl1_config->id_primary_cluster,
            MOD_PD_SET_STATE_NO_RESP,
            MOD_PD_STATE_ON);
        ctx.mod_pd_restricted_api->set_state(
            ctx.bl1_config->id_primary_core,
            MOD_PD_SET_STATE_NO_RESP,
            MOD_PD_STATE_ON);
    }
...
static int tc4_bl1_process_notification(
    const struct fwk_event *event,
    struct fwk_event *resp_event)
{
    fwk_assert(fwk_id_is_equal(event->id, mod_tc4_bl1_notification_id_systop));
...
static int tc4_bl1_process_bind_request(
    fwk_id_t source_id,
    fwk_id_t target_id,
    fwk_id_t api_id,
    const void **api)
{
    *api = &tc4_system_transport_firmware_signal_api;
    return FWK_SUCCESS;
}

/* Module descriptor */
const struct fwk_module module_tc4_bl1 = {
    .api_count = MOD_TC4_BL1_API_IDX_COUNT,
    .type = FWK_MODULE_TYPE_SERVICE,
    .event_count = MOD_BL1_EVENT_COUNT,
    .notification_count = MOD_TC4_BL1_NOTIFICATION_COUNT,
    .init = tc4_bl1_init,
    .bind = tc4_bl1_bind,
    .start = tc4_bl1_start,
    .process_bind_request = tc4_bl1_process_bind_request,
    .process_event = tc4_bl1_process_event,
    .process_notification = tc4_bl1_process_notification,
};
