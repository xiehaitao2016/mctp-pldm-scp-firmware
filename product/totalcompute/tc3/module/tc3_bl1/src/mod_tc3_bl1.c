...
struct tc3_bl1_ctx {
    const struct mod_tc3_bl1_config *bl1_config;
...
static int tc3_signal_error(fwk_id_t service_id)
{
    fwk_unexpected();
    return FWK_E_PANIC;
}
...
static int tc3_signal_message(fwk_id_t service_id)
{
    struct fwk_event event = {
        .source_id = FWK_ID_MODULE(FWK_MODULE_IDX_TC3_BL1),
        .target_id = FWK_ID_MODULE(FWK_MODULE_IDX_TC3_BL1),
        .id = FWK_ID_EVENT(FWK_MODULE_IDX_TC3_BL1, MOD_BL1_EVENT_RSS_HANDSHAKE),
    };
...
static const struct mod_transport_firmware_signal_api
    tc3_system_transport_firmware_signal_api = {
        .signal_error = tc3_signal_error,
        .signal_message = tc3_signal_message,
    };
...
static int bl1_deferred_setup(void)
{
    ...
    FWK_LOG_INFO("[TC3_BL1] Pinging RSS");
...
static int tc3_bl1_bind(fwk_id_t id, unsigned int round)
{
    int status;
...
static int tc3_bl1_start(fwk_id_t id)
{
    struct fwk_event event = {
        .source_id = FWK_ID_MODULE(FWK_MODULE_IDX_TC3_BL1),
        .target_id = FWK_ID_MODULE(FWK_MODULE_IDX_TC3_BL1),
        .id = FWK_ID_EVENT(FWK_MODULE_IDX_TC3_BL1, MOD_BL1_EVENT_RUN),
    };

    return fwk_put_event(&event);
}

static int tc3_bl1_process_event(
    ...
    struct fwk_event *resp)
{
    ...
    if (fwk_id_get_event_idx(event->id) == MOD_BL1_EVENT_RUN) {
        ...
        struct fwk_event systop_on_event = {
            ...
            .id = mod_tc3_bl1_notification_id_systop,
        ...
        FWK_LOG_INFO("[TC3_BL1] Got ACK from RSS");
        ...
        FWK_LOG_CRIT(
            "[TC3_BL1] Failed to load RAM firmware image: %s",
...
static int tc3_bl1_process_notification(
    const struct fwk_event *event,
    struct fwk_event *resp_event)
{
    fwk_assert(fwk_id_is_equal(event->id, mod_tc3_bl1_notification_id_systop));
...
static int tc3_bl1_process_bind_request(
    fwk_id_t source_id,
    fwk_id_t target_id,
    fwk_id_t api_id,
    const void **api)
{
    *api = &tc3_system_transport_firmware_signal_api;
    return FWK_SUCCESS;
}

/* Module descriptor */
const struct fwk_module module_tc3_bl1 = {
    .api_count = MOD_TC3_BL1_API_IDX_COUNT,
    .type = FWK_MODULE_TYPE_SERVICE,
    .event_count = MOD_BL1_EVENT_COUNT,
    .notification_count = MOD_TC3_BL1_NOTIFICATION_COUNT,
    .init = tc3_bl1_init,
    .bind = tc3_bl1_bind,
    .start = tc3_bl1_start,
    .process_bind_request = tc3_bl1_process_bind_request,
    .process_event = tc3_bl1_process_event,
    .process_notification = tc3_bl1_process_notification,
};
