...
static int scmi_sys_power_state_notify_handler(fwk_id_t service_id,
    const uint32_t *payload);
static int scmi_sys_power_state_notifier_handler(fwk_id_t service_id,
    const uint32_t *payload);
#endif
...
#    ifdef BUILD_HAS_NOTIFICATION
    ...
    struct fwk_event notification_event = {
        .id = mod_scmi_system_power_notification_system_power_down,
        .response_requested = false,
        .source_id = FWK_ID_MODULE_INIT(FWK_MODULE_IDX_SCMI_SYSTEM_POWER),
    };
    fwk_str_memcpy(
        notification_event.params,
        &mod_scmi_system_state,
        sizeof(mod_scmi_system_state));
    *status = fwk_notification_notify(&notification_event, &count);
    if (*status != FWK_SUCCESS) {
        FWK_LOG_ERR(
            "[SCMI_SYSTEM_POWER] failed to notify power state "
            "transition: %s",
 ...
 */
static int scmi_sys_power_notification_handler(fwk_id_t protocol_id,
    fwk_id_t service_id,
    const uint32_t *payload,
    size_t payload_size,
    unsigned int message_id)
{
    ...
    static_assert(FWK_ARRAY_SIZE(notification_handler_table) ==
        FWK_ARRAY_SIZE(payload_size_table_notification),
        "[SCMI] System power protocol notification table sizes not consistent");

    fwk_assert(payload != NULL);

    if (message_id >= FWK_ARRAY_SIZE(notification_handler_table)) {
        return_value = (int32_t)SCMI_NOT_FOUND;
        goto error;
    }

    if (payload_size != payload_size_table_notification[message_id]) {
        /* Incorrect payload size or message is not supported */
        return_value = (int32_t)SCMI_PROTOCOL_ERROR;
        goto error;
    }

    return notification_handler_table[message_id](service_id, payload);
